...
int main(void)
{
	...
	/*----------------------- 文件系统测试：写测试-------------------*/
	printf("\r\n****** 即将进行文件写入测试... ******\r\n");
	res_flash = f_open(&fnew, "1:FatFs 读写文件测试.txt", FA_CREATE_ALWAYS | FA_WRITE);
	if (res_flash == FR_OK)
	{
		printf("》》打开/创建FatFs读写测试文件.txt 文件成功， 向文件写入数据\r\n");
	...
	/*------------------- 文件系统测试：读测试--------------------------*/
	printf("******* 即将进行文件读取测试**********\r\n");
	res_flash = f_open(&fnew, "1:FatFs 读写文件测试.txt",FA_OPEN_EXISTING | FA_READ);
